...
inline auto get_initial_sample_adj_hash_map(const std::vector<int32_t>& n_ids) {
  phmap::flat_hash_map<int32_t, int32_t> n_id_map;
...
inline SingleSample sample_adj(torch::Tensor rowptr, torch::Tensor col,
  ...
  std::vector<std::vector<int32_t>> cols(idx_size);
  ...
  const auto expand_neighborhood = [&](auto add_neighbors) -> void {
    for (size_t i = 0; i < idx_size; ++i) {
      ...
      const auto add_neighbor = [&](const int32_t p) -> void {
        ...
        cols[i].push_back(ins.first->second);
      };
  ...
  if (num_neighbors <
      0) {  // No sampling ======================================
    expand_neighborhood([](const int32_t neighbor_count, auto add_neighbor) {
      for (int32_t j = 0; j < neighbor_count; j++) {
        add_neighbor(j);
                         ...
                         // =============================
    expand_neighborhood(
        [num_neighbors](const int32_t neighbor_count, auto add_neighbor) {
    ...
    expand_neighborhood([num_neighbors, &perm](const int32_t neighbor_count,
                                               auto add_neighbor) {
      perm.clear();
      ...
      if (neighbor_count <= num_neighbors) {
        ...
        for (int32_t j = neighbor_count - num_neighbors; j < neighbor_count;
             j++) {
          const int32_t option = gen() % j;
          auto winner = option;
  ...
  auto out_e_id = torch::empty(0, col.options().pinned_memory(pin_memory));
  const auto out_e_id_data = out_e_id.data_ptr<int64_t>();
...
sample_adj(torch::Tensor rowptr, torch::Tensor col, torch::Tensor idx,
           int32_t num_neighbors, bool replace, bool pin_memory = false) {
